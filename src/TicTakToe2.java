...
public class TicTakToe2 {
    ...
    private static int dot_x = -1;
    private static int dot_y = -1;
                ...
                if ((dot_x - 1) < 0) {
                    if ((dot_y + 1) >= SIZE) {
                        if ((dot_y - 1) < 0) {
                            ...
                            if (isCellValid(dot_x, (dot_y - 1))) {
                                dot_y = dot_y - 1;
                                System.out.println("Компьютер выбрал ячейку " + (dot_y + 1) + " " + (dot_x + 1));
                                map[dot_y][dot_x] = DOT_O;
